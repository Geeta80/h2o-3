...
public class GLMModel extends Model<GLMModel,GLMModel.GLMParameters,GLMModel.GLMOutput> implements Model.Contributions{
 ...
 ///    assert !_parms._standardize; // -------------------------------------!!!!!!!!!!!!!!
    ...
    double[] means = new double[_output.beta().length - (_output._dinfo._intercept?1:0)];
    int featureIdx = 0;
    for (int i = 0; i < means.length; i++) {
      Vec col = adaptedFrame.vec(_output._coefficient_names[i]);
      if (col != null) {
        means[i] = col.mean();
        featureIdx++;
      } else {
        col = adaptedFrame.vec(names()[featureIdx]);
        double[] frequencies = VecUtils.collectDomainWeights(col, null);
        int skip=0;
        if (! (_output._coefficient_names[i].equals(names()[featureIdx]+"."+_output._domains[featureIdx][0])) &&
                (_output._coefficient_names[i].equals(names()[featureIdx]+"."+_output._domains[featureIdx][1])))
          skip++;
        for (int j = skip; j < frequencies.length; j++) {
          assert (_output._coefficient_names[i+j-skip].equals(names()[featureIdx]+"."+_output._domains[featureIdx][j]));
          means[i + j - skip] = frequencies[j]/col.length();
        }
        i += adaptedFrame.domains()[featureIdx].length - 1 - skip;
        featureIdx++;
      }      
    }
